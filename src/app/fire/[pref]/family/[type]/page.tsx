...
import { FAMILY_TYPES_FOR_SEO, FAMILY_COEFFICIENTS } from "@/config/assumptions";
import { generateFamilyCases } from "@/lib/seo-helpers";
...
export function generateStaticParams() {
  ...
  for (const p of prefectures) {
    for (const ft of FAMILY_TYPES_FOR_SEO) {
      params.push({ pref: p.code, type: ft.key });
    }
  }
  return params;
}
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: { title, description, url: `/fire/${pref}/family/${type}/`, siteName: "FIREシミュレーター" },
  };
}
...
export default async function FamilyPage({
  ...
  const cases = generateFamilyCases(pref, type);
    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      <JsonLd
        data={{
          ...
          "@type": "WebPage",
          name: `${prefecture.name}・${familyLabel}のFIREシミュレーション`,
          url: `${SITE_URL}/fire/${pref}/family/${type}/`,
      ...
      <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">
        {prefecture.name}・{familyLabel}のFIREシミュレーション
      </h1>
      <p className="mt-2 text-gray-600">
        {prefecture.name}（生活費係数{prefecture.costIndex}）で{familyLabel}
        の場合、年収によってFIRE必要資産と達成年がどう変わるか比較します。
      </p>

      <div className="mt-6 rounded-lg border border-primary-200 bg-primary-50 p-4 text-center">
        <Link
          href={`/simulate/?pref=${pref}&family=${type}`}
